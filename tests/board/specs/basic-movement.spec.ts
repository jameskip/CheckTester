...
import { extractScenarioTestData, calculatePosition } from '../../shared/utils'

const scenarioData = extractScenarioTestData(boardScenarios.initial)
...
test.describe('TC1: Basic Piece Selection and Movement', () => {
  test.describe('Initial Board State', () => {
    test('TC1.1: should display initial board with 12 pieces per player', async ({ checkersPage: page }) => {
      ...
      await Actions.verifyBoardState(page, scenarioData.boardState)
    })
  })

  test.describe('Piece Selection', () => {
    test('TC1.2: should allow selecting and deselecting orange pieces', async ({ checkersPage: page }) => {
      await Actions.selectPiece(page, scenarioData.firstMoveFrom)
      await Actions.verifyPieceSelection(page, scenarioData.firstMoveFrom, true)

      await Actions.deselectPiece(page, scenarioData.firstMoveFrom)
      await Actions.verifyPieceSelection(page, scenarioData.firstMoveFrom, false)
    })

    test('TC1.3: should show visual feedback when switching piece selection', async ({ checkersPage: page }) => {
      await Actions.selectPiece(page, scenarioData.firstMoveFrom)
      await expect(Locators.selectedPiece(page, scenarioData.firstMoveFrom)).toBeVisible()

      await Actions.selectPiece(page, scenarioData.secondMoveFrom)
      await expect(Locators.selectedPiece(page, scenarioData.firstMoveFrom)).toBeHidden()
      await expect(Locators.selectedPiece(page, scenarioData.secondMoveFrom)).toBeVisible()
    })

    test('TC1.4: should prevent selecting opponent blue pieces', async ({ checkersPage: page }) => {
      await Actions.attemptInvalidPieceSelection(page, scenarioData.firstBluePosition, gameMessages.opponentPieceError)
      await expect(Locators.piece(page, scenarioData.firstBluePosition, PIECE_COLORS.BLUE)).toBeVisible()
    })
  })

  test.describe('Valid Movements', () => {
    test('TC1.5: should allow valid forward diagonal moves', async ({ checkersPage: page }) => {
      await Actions.selectPiece(page, scenarioData.firstMove.from)
      await Locators.clickableSquare(page, scenarioData.firstMove.to).click()

      await Actions.verifyPieceAtPosition(page, scenarioData.firstMove.to, PIECE_COLORS.ORANGE)
      await Actions.verifyEmptySquare(page, scenarioData.firstMove.from)
    })

    test('TC1.6: should execute all valid moves in sequence', async ({ checkersPage: page }) => {
      for (const move of scenarioData.allMoves) {
  ...
  test.describe('Invalid Movements', () => {
    test('TC1.7: should reject invalid backward diagonal moves', async ({ checkersPage: page }) => {
      const invalidBackwardMove = calculatePosition(scenarioData.firstMoveFrom, DIRECTION_OFFSETS.BACKWARD_RIGHT)

      await Actions.selectPiece(page, scenarioData.firstMoveFrom)
      await Locators.clickableSquare(page, invalidBackwardMove).click()

      await Actions.verifyPieceAtPosition(page, scenarioData.firstMoveFrom, PIECE_COLORS.ORANGE)
    })

    test('TC1.8: should reject moves to occupied squares', async ({ checkersPage: page }) => {
      await Actions.selectPiece(page, scenarioData.firstMoveFrom)
      await Locators.clickableSquare(page, scenarioData.secondOrangePosition).click()
  ...
  test.describe('Game State Management', () => {
    test('TC1.9: should keep game in progress after valid move', async ({ checkersPage: page }) => {
      await Actions.performPlayerTurn(page, scenarioData.firstMove)
    ...
    test('TC1.10: should maintain blue pieces count after orange moves', async ({ checkersPage: page }) => {
      await Actions.movePiece(page, scenarioData.firstMove.from, scenarioData.firstMove.to)
